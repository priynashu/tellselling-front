...
class CreateRoom3 extends Component {
  ...
  Save = () => {
    ...
    Update = () => {
      ...
      axios.post('http://localhost:5000/salesroom-live-stream/update', salesroom).then((res) => {
        toast.success('Salesroom saved', {
          position: toast.POSITION.BOTTOM_LEFT,
        });
      }).catch(err => console.log(err));
  ...
  GetDataIfEdit(){
    ...
    if(query.length!=0){
      ...
      const localUser = getLocalUser();
      const link='http://localhost:5000/salesroom-live-stream/'+localUser.tenantId+"/"+salesroom_link;
      axios.get(link)
      .then((res)=>{
        const data=res.data;
