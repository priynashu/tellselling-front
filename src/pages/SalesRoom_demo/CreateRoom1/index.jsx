...
class CreateRoom1 extends Component {
  ...
  GetDataIfEdit() {
    ...
    if (query.length != 0) {
      ...
      const localUser = getLocalUser();
      const link = backend_url + 'salesroom/' + localUser.tenantId + "/" + query.substr(1, query.length);
      axios.get(link)
        .then((res) => {
          const data = res.data;
          this.setState({ salesroom_id: data._id });
          this.setState({ prospectus: data.prospects });
          this.setState({ info: { name: data.title, thumbnail: data.thumbnail, description: data.description, tags: data.tags } })
