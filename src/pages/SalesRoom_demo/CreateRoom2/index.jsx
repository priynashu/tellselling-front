...
class CreateRoom2 extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      info: { eventName: "", thumbnail: "", eventPlace: "", meetingValues: {}, eventType:"",link: Math.random().toString(36).slice(2), platform:'',stream_id:'',dateTime:"",sales_person:"",access:{},fullDate:""},
      contents: [],
  ...
  Save = () => {
    ...
    axios.post('http://localhost:5000/salesroom-webinar/create', salesroom).then((res) => {
    ...
    Update = () => {
      ...
      axios.post('http://localhost:5000/salesroom-webinar/update', salesroom).then((res) => {
        toast.success('Salesroom saved', {
          position: toast.POSITION.BOTTOM_LEFT,
        });
      }).catch(err => console.log(err));
  ...
  GetDataIfEdit(){
    ...
    if(query.length!=0){
      ...
      const localUser = getLocalUser();
      const link='http://localhost:5000/salesroom-webinar/'+localUser.tenantId+"/"+salesroom_link;
      axios.get(link)
      .then((res)=>{
        const data=res.data;
        ...
        this.setState({info: { title: data.title, thumbnail: data.thumbnail, tags: data.tags, link: data.link, eventPlace: data.eventPlace, eventType: data.eventType, meetingValues:data.meetingValues,dateTime:data.dateTime,fullDate:data.fullDate }})
  ...
  render() {
    ...
    const ManageScreen = () =>{
      return (<Manage UpdateScreen={UpdateScreen} UpdateInfo={UpdateInfo} updateQuestion={updateQuestion} InfoData={this.state.info} />);
    }
