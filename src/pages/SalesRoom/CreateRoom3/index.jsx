...
class CreateRoom3 extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      info: { name: "", thumbnail: "", description: "", tags: [], link: Math.random().toString(36).slice(2), platform:'',stream_id:'',events:[],fullDate:"",dateTime:""},
      contents: [],
  ...
  Save = () => {
    ...
    axios.post(backend_url+`salesroom-live-stream/create`, salesroom).then((res) => {
    ...
    axios.post(backend_url+`salesroom-live-stream/publish/:${this.state.info.link}`, publishData).then((res) => {
      console.log("live publish data saved");
    }).catch(err => console.log(err));
  }
    ...
    Update = () => {
      ...
      console.log("updated salesroom",salesroom);
      axios.post(backend_url+'salesroom-live-stream/update', salesroom).then((res) => {
        toast.success('Salesroom saved', {
          position: toast.POSITION.BOTTOM_LEFT,
        });
      }).catch(err => console.log(err));
  ...
  GetDataIfEdit(){
    ...
    if(query.length!=0){
      ...
      const localUser = getLocalUser();
      const link=backend_url+'salesroom-live-stream/'+localUser.tenantId+"/"+salesroom_link;
      axios.get(link)
      .then((res)=>{
        const data=res.data;
        ...
        this.setState({info: { name: data.title, thumbnail: data.thumbnail, description: data.description, tags: data.tags, link: data.link, platform: data.platform, stream_id: data.stream_id, events:data.events,dateTime:data.dateTime,fullDate:data.fullDate }})
      ...
      axios.post(backend_url+`salesroom-live-stream/publish/:${this.state.info.link}`, publishData).then((res) => {
        const data = res.data
        console.log("edit publish live",data);
  ...
  render() {
    ...
    const UpdateWelcomeMessage = (welcome_message) => {
      console.log("index",welcome_message);
      this.setState({ welcome_message:welcome_message });
    }
    ...
    const ManageScreen = () =>{
      return (<Manage UpdateScreen={UpdateScreen} UpdateInfo={UpdateInfo} updateQuestion={updateQuestion} InfoData={this.state.info} question={this.state.question} />);
    }
