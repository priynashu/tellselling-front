 ...
 */
    ...
    render() {
        console.log("link 2",backend_url);
        if(this.state.previewBtn===true){
            let data=[]
             axios.get(backend_url+`salesroom-webinar/${this.state.link}`).then((res)=>{
                
                data=res.data
                console.log("inside saves preview",data,res);
                if(data!=null){
                    this.setState({previewBtn:false})
                }
                console.log("inside if web",this.state.previewBtn);
            })
        }
        ...
        const SaveSalesroom = async() => {            
            ...
            var promise = setTimeout(()=>{
             ...
             axios.get(backend_url+`salesroom-webinar/${this.state.link}`).then((res)=>{
                ...
                data=res.data
                console.log("inside saves",data,res);
                if(data.length!=0){
                    this.setState({previewBtn:false})
                }
        ...
        const handlePreviewClick=async()=>{
            ...
            if(this.state.previewBtn===false){
                window.open(backend_url+`salesroom-web/:${this.state.link}`,"","width=1000,height=1000")
            }
            ...
            <div className={styles.main} style={{ marginBottom: 150 }}>
                <div className={styles.header}>
                    ...
                    <div className={styles.header_buttons}>
                        ...
                        <Body updateDomains={this.props.updateDomains} domains={this.props.domains}/>
                    </div>
                </div>
                <div className={styles.filter_div}>
                    ...
                    {this.state.availableCategories?.map((topic, key) => {
                        return (
                            <p onClick={() => { SelectCategory(topic.name); }}
