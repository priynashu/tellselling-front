 ...
 */
    ...
    render() {
        // console.log("content data live",this.props.info);
        if(this.state.previewBtn===true){
            let data=[]
             axios.get(`http://localhost:5000/salesroom-live-stream/${this.state.link}`).then((res)=>{
                
                data=res.data
                console.log("inside saves",data);
                if(data!=null){
                    this.setState({previewBtn:false})
                }
                console.log("inside if",this.state.previewBtn);
            })
        }
        ...
        //Save salesroom data
            ...
            const SaveSalesroom = async() => {            
                await this.props.Save(); //Pass to parent
                var promise = setTimeout(()=>{
                    let data=[]
                 axios.get(`http://localhost:5000/salesroom-live-stream/${this.state.link}`).then((res)=>{
                    
                    data=res.data
                    console.log("inside saves",res);
                    if(data.length!=0){
                        this.setState({previewBtn:false})
                    }
                    console.log("preview btn",this.state.previewBtn);
                })
                },2000)
            }
        ...
        const handlePreviewClick=async()=>{
            ...
            if(this.state.previewBtn===false){
                window.open(`http://localhost:5000/salesroom-live/:${this.state.link}`,"","width=1500,height=1500")
            }
            ...
            <div className={styles.main} style={{ marginBottom: 150 }}>
                <div className={styles.header}>
                    ...
                    <div className={styles.header_buttons}>
                        <Button onClick={() => { this.props.UpdateScreen("MANAGE") }} variant='outlined'>Manage</Button>
                        <Button onClick={handlePreviewClick} disabled={this.state.previewBtn} variant='outlined'>Preview</Button>
                        {/* <Button onClick={()=>{window.open(`http://localhost:5000/salesroom-live/:${this.state.link}`)}} variant='outlined'>Preview</Button> */}
