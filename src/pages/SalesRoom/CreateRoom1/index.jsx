...
class CreateRoom1 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      //Important datas to create the salesroom
      info: { name: "", thumbnail: "", description: "", tags: [], link: Math.random().toString(36).slice(2),fullDate:"" },
      contents: [],
  ...
  Save = () => {
    // console.log("inside save basic",this.state.salesroom_id);
    ...
    axios.post(backend_url + 'salesroom/create', salesroom).then((res) => {
    ...
    const publishData = {
      ...
      link:this.state.info.link,
      type:"salesroom-basic",      
      method:'create'
    }
    axios.post(backend_url+`salesroom/publish/:${this.state.info.link}`, publishData).then((res) => {
      this.setState({ salesroom_id: res.data._id });
      console.log("basic publish data saved");
    }).catch(err => console.log(err));
  }

  //Update content to server
  Update = () => {
    // console.log("inside update basic",this.state.salesroom_id);
    const localUser = getLocalUser();
    const salesroom = {
      tenantId: localUser.tenantId,
    ...
    axios.post(backend_url+'salesroom/update', salesroom).then((res) => {
      toast.success('Salesroom saved', {
        position: toast.POSITION.BOTTOM_LEFT,
      });
    }).catch(err => console.log(err));
  ...
  GetDataIfEdit() {
    ...
    if (query.length != 0) {
      ...
      const localUser = getLocalUser();
      const link = backend_url + 'salesroom/' + localUser.tenantId + "/" + salesroom_link;
      axios.get(link)
        .then((res) => {
          const data = res.data;
          this.setState({ salesroom_id: data._id });
          this.setState({ prospectus: data.prospects });
          this.setState({ info: { name: data.title, thumbnail: data.thumbnail, description: data.description, tags: data.tags,link:data.link } })
